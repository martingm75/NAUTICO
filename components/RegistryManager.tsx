...
import { Search, Plus, User, Ruler, Ship, MapPin, Save, X, Trash2, Anchor, ArrowRight, History, Compass, CheckCircle2 } from 'lucide-react';
...
type RegistryTab = 'base_current' | 'base_past' | 'transit';
...
const RegistryManager: React.FC<RegistryManagerProps> = ({ 
  ...
  const [activeTab, setActiveTab] = useState<RegistryTab>('base_current');
  ...
  const filteredRegistry = useMemo(() => {
    ...
    return textFiltered.filter(boat => {
      ...
      if (activeTab === 'base_current') {
        return boat.isBase && isActive;
      }
      if (activeTab === 'base_past') {
        return boat.isBase && !isActive;
      }
      if (activeTab === 'transit') {
        return !boat.isBase; // Mostramos todos los de tránsito (histórico y actuales) juntos, o solo activos si se prefiere.
                             // Por lo general tránsito es un log histórico, así que mostramos todos.
      }
      return true;
    });
  ...
  const counts = useMemo(() => {
    const base = registry.filter(b => b.isBase);
    const transit = registry.filter(b => !b.isBase);
    return {
  ...
  const handleOpenModal = (boat?: Boat) => {
    if (boat) {
      ...
      setFormData({
        ...
        departureDate: '',
        isBase: activeTab.includes('base'), // Pre-marcar base si estamos en pestaña base
      });
    }
    setIsModalOpen(true);
  };
    ...
    <div className="bg-white rounded-2xl shadow-sm border border-slate-200 h-full flex flex-col overflow-hidden">
      ...
      <div className="p-6 border-b border-slate-100 flex justify-between items-center bg-slate-50 shrink-0">
        <div>
          ...
          <p className="text-sm text-slate-500 mt-1">Gestión de flota Base y Tránsito.</p>
        </div>
        <button 
      ...
      <div className="flex border-b border-slate-200">
        <TabButton id="base_current" label="Base (Actuales)" icon={Anchor} colorClass="text-emerald-600 border-emerald-600" />
        ...
        {filteredRegistry.length === 0 ? (
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 gap-4">
            {filteredRegistry.map(boat => {
              const isActive = activeBoatIds.includes(boat.id);
              return (
                <div key={boat.id} className={`group bg-white border rounded-2xl p-5 hover:shadow-xl transition-all relative ${boat.isBase ? 'border-emerald-100 hover:border-emerald-300' : 'border-amber-100 hover:border-amber-300'}`}>
                  <div className="flex justify-between items-start mb-3">
                    <div className={`w-10 h-10 rounded-xl flex items-center justify-center ${boat.isBase ? 'bg-emerald-100 text-emerald-600' : 'bg-amber-100 text-amber-600'}`}>
                      {boat.isBase ? <Anchor size={20} /> : <Compass size={20} />}
                    </div>
                  ...
                  <div className="flex items-center gap-2 mb-1">
                     ...
                     {isActive && <CheckCircle2 size={16} className="text-emerald-500" title="Actualmente en puerto" />}
                  </div>
                  ...
                  {isActive ? (
                    <div className="w-full py-2 bg-emerald-50 text-emerald-700 rounded-xl text-xs font-black uppercase text-center border border-emerald-100">
                      En Puerto
                    </div>
                  ) : (
                    <button 
                      onClick={() => handleAssignClick(boat)}
                      className="w-full py-2 bg-slate-50 text-slate-600 rounded-xl text-xs font-black uppercase hover:bg-sky-500 hover:text-white transition-all flex items-center justify-center gap-2 group-hover:bg-sky-500 group-hover:text-white"
                    >
                      <MapPin size={14} /> Ubicar en Puerto
                    </button>
                  )}
                </div>
              );
